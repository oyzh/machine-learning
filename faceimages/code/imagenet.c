...
BPNN *net;
{
  ...
  switch(head[0])
    {
    case 'r':net->target[1] = TARGET_HIGH;
      break;
    case 's':net->target[2] = TARGET_HIGH;
      break;
    case 'u':net->target[3] = TARGET_HIGH;
      break;
    case 'l':net->target[4] = TARGET_HIGH;
      break;
    default:
      printf("Head has no %s",head);
      return;
    }
  printf("%c\n",head[0]);
}
